									  ...
									  struct ftrace_in *ftrace_in)
{
	...
	ftrace_in->trace_identity = (struct ftrace_trace_identity){
		.namespace = ftrace_in->options.config.lttng_format ? LTTNG_NAMESPACE :
															  FTRACE_NAMESPACE,
							 ...
							 const char *buffer_name, int buffer_index)
{
	 ...
	 */
	const struct ftrace_common_config config = get_common_config(ftrace_in);
	tc_buffer->stream_class = ftrace_create_stream_class(
		ftrace_in->trace_class, ftrace_in->clock_class, USE_PACKAGES,
		buffer_index, ftrace_in->trace_uid,
		ftrace_in->options.config.lttng_format);
	if (buffer_name)
	...
	for (int i = 0; i < ncpus; ++i) {
		...
		char *stream_name = ftrace_format_stream_name(ftrace_in->tracedat_path,
